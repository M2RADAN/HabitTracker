...
**Основная цель:**
Разработать и выпустить в **MVP (минимально жизнеспособный продукт) приложения "Habit Tracker"** к концу Q1 2026 года, который позволит пользователям создавать, отслеживать и анализировать свои привычки.

- **Specific (Конкретная):** Создать работающий MVP приложения с ключевым функционалом (создание, отслеживание привычек, визуализация, мотивация).
...
## Статус проекта: В разработке (в прогрессе)

Проект в активной фазе разработки. **Этап 1 (Ядро приложения) полностью завершен!** Этап 2 (Визуализация и Мотивация) — в прогрессе: реализована логика "стриков", базовый экран "Прогресс" с отображением статистики и встроенным heatmap (временная реализация). На текущем спринте завершена базовая система достижений: создан список ачивок, реализована логика проверки условий, добавлены UI (модалка подробностей), локальные уведомления и in-app тосты; также внедрён контекст для синхронизации ачивок между экранами. Работа продолжается над визуальными деталями, расширением набора ачивок и улучшением UX.
...
- [✅] **📦 Этап 1: Ядро приложения (Core Functionality)**
  ...
  - [✅] 1.4. Разработка экрана создания/редактирования привычки
    - [✅] 1.4.1. Форма с полями (название, тип, частота, измеримость)
    - [✅] 1.4.2. Логика сохранения новой привычки
  - [✅] 1.5. Реализация базовой логики отслеживания
    - [✅] 1.5.1. Отметка выполнения (checkbox)
    - [✅] 1.5.2. Увеличение счётчика (counter)
  - [✅] 1.6. Локальное хранение данных (AsyncStorage)

- [ ] **✨ Этап 2: Визуализация и Мотивация (In progress)**
  ...
  - [~] 2.2. Разработка экрана прогресса (в прогрессе)
    - [✅] 2.2.1. Интеграция календаря-графика (встроенный heatmap, временная реализация)
    - [~] 2.2.2. Отображение общей статистики (процент выполнений и т.д.) — частично реализовано
  - [~] 2.3. Внедрение системы достижений (ачивки)
    - [✅] 2.3.1. Создание списка ачивок и условий их получения — выполнено
    - [✅] 2.3.2. Логика проверки и разблокировки ачивок + уведомления (локальные) — выполнено
    - [~] 2.3.3. UI для отображения и детализации ачивок (модалка, тосты) — выполнено, UX дорабатывается
...
| **2** | **Визуализация**                   |           |          |           |       |    ~60%    |
| 2.1   | Реализация системы "стриков"       | Ксения А. |          |           |   0   |    100%    |
| 2.2   | Разработка экрана прогресса        | Ксения А. |          |           |   0   |    60%     |
| 2.3   | Внедрение системы достижений       | Ксения А. |          |           |   0   |    80%     |
| 2.4   | Создание библиотеки шаблонов       | Ксения А. |          |           |   0   |     0%     |
| **3** | **Интеграции и QoL**               |           |          |           |       |     0%     |
...
Следующие шаги — в приоритете на ближайшие спринты:

- Доработать визуальные детали экрана "Прогресс": добавить более точную статистику, фильтры по периоду и переход к деталям привычки.
- Расширить набор ачивок: добавить специфические цели по привычкам, визуальные награды и Lottie-анимации для ключевых ачивок.
- Улучшить UX уведомлений: добавить mute control, возможность просматривать историю ачивок и подробную страницу ачивки.
- Написать unit-тесты для core-логики ачивок (evaluate, evaluateAndNotify) и интеграционные тесты для основных экранов.

Задача по синхронизации ачивок между экранами решена через контекст: см. [`HabitTrackerV2/app/utils/AchievementsContext.tsx:1`] и интеграцию в главный экран/таб достижений.

---

Если нужно — могу обновить также раздел "Дерево целей" с более детальными задачами по каждой ачивке или добавить текстовую дорожную карту (milestones) на ближайшие 4 спринта.
